...
let () =
  ...
  gl_buffer_data GL_ARRAY_BUFFER (9 * 4) vertices GL_STATIC_DRAW |> ignore;
  gl_vertex_attrib_pointer 0 3 GL_FLOAT false (3 * 4) 0 |> ignore;
  ...
  while not (GLFW.windowShouldClose ~window:window) do
    ...
    (* draw (); *)
    gl_use_program shader_program |> ignore;
    gl_draw_arrays GL_TRIANGLES 0 3 |> ignore;
