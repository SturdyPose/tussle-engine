module Constants = struct
  ...
  let gl_TIMEOUT_IGNORED = 0xFFFFFFFFFFFFFFF
...
external glew_init: unit -> bool = "glew_init";;
external gl_clear_color: r:float -> g:float -> b:float -> a:float -> bool = "gl_clear_color";;
...
let mask_to_int = function
  | GL_COLOR_BUFFER_BIT   -> Constants.gl_COLOR_BUFFER_BIT
  | GL_DEPTH_BUFFER_BIT   -> Constants.gl_DEPTH_BUFFER_BIT
  | GL_STENCIL_BUFFER_BIT -> Constants.gl_STENCIL_BUFFER_BIT

external c_gl_clear: int -> bool = "gl_clear";;
let gl_clear = function
  | [] -> false
  | x :: rest -> let res = List.fold_left (fun acc y -> mask_to_int y lor acc ) (mask_to_int x) rest in c_gl_clear res;;
(* let gl_clear2 mask1 mask2 = mask_to_int mask1 lor mask_to_int mask2 |> c_gl_clear
let gl_clear3 mask1 mask2 mask3 = mask_to_int mask1 lor mask_to_int mask2 lor mask_to_int mask3 |> c_gl_clear *)
...
let mode_to_int = function
 | GL_POINTS -> Constants.gl_POINTS
 | GL_LINE_STRIP -> Constants.gl_LINE_STRIP
 | GL_LINE_LOOP -> Constants.gl_LINE_LOOP
 | GL_LINES -> Constants.gl_LINES 
 | GL_LINE_STRIP_ADJACENCY -> Constants.gl_LINE_STRIP_ADJACENCY 
 | GL_LINES_ADJACENCY -> Constants.gl_LINES_ADJACENCY 
 | GL_TRIANGLE_STRIP -> Constants.gl_TRIANGLE_STRIP 
 | GL_TRIANGLE_FAN -> Constants.gl_TRIANGLE_FAN 
 | GL_TRIANGLES -> Constants.gl_TRIANGLES 
 | GL_TRIANGLE_STRIP_ADJACENCY -> Constants.gl_TRIANGLE_STRIP_ADJACENCY 
 | GL_TRIANGLES_ADJACENCY  -> Constants.gl_TRIANGLES_ADJACENCY  
 | GL_PATCHES  -> Constants.gl_PATCHES

external c_gl_draw_arrays: mode:int -> first:int -> size:int -> bool = "gl_draw_arrays";;
let gl_draw_arrays mode first size = let m = mode_to_int mode in c_gl_draw_arrays ~mode:m ~first:first ~size:size
...
let shader_type_to_int = function
  | GL_COMPUTE_SHADER           -> Constants.gl_COMPUTE_SHADER
  | GL_VERTEX_SHADER            -> Constants.gl_VERTEX_SHADER
  | GL_TESS_CONTROL_SHADER      -> Constants.gl_TESS_CONTROL_SHADER
  | GL_TESS_EVALUATION_SHADER   -> Constants.gl_TESS_EVALUATION_SHADER
  | GL_GEOMETRY_SHADER          -> Constants.gl_GEOMETRY_SHADER
  | GL_FRAGMENT_SHADER          -> Constants.gl_FRAGMENT_SHADER
  
(* glCreateShader returns uint32 which fits in int *)
external c_gl_create_shader: int -> int = "gl_create_shader";;
let gl_create_shader shader_type = shader_type_to_int shader_type |> c_gl_create_shader

external gl_gen_buffers: int -> int array = "gl_gen_buffers";;
external gl_gen_buffer: unit -> int = "gl_gen_buffer";;

external gl_gen_vertex_arrays: int -> int array = "gl_gen_vertex_arrays";;
external gl_gen_vertex_array: unit -> int = "gl_gen_vertex_array";;
...
let buffer_type_to_int = function
  | GL_ARRAY_BUFFER -> Constants.gl_ARRAY_BUFFER
  (* | GL_ATOMIC_COUNTER_BUFFER -> Constants.gl_ATOMIC_COUNTER_BUFFER *)
  | GL_COPY_READ_BUFFER -> Constants.gl_COPY_READ_BUFFER
  | GL_COPY_WRITE_BUFFER -> Constants.gl_COPY_WRITE_BUFFER
  (* | GL_DISPATCH_INDIRECT_BUFFER -> Constants.gl_DISPATCH_INDIRECT_BUFFER *)
  (* | GL_DRAW_INDIRECT_BUFFER -> Constants.gl_DRAW_INDIRECT_BUFFER *)
  | GL_ELEMENT_ARRAY_BUFFER -> Constants.gl_ELEMENT_ARRAY_BUFFER
  | GL_PIXEL_PACK_BUFFER -> Constants.gl_PIXEL_PACK_BUFFER
  | GL_PIXEL_UNPACK_BUFFER -> Constants.gl_PIXEL_UNPACK_BUFFER
  (* | GL_QUERY_BUFFER -> Constants.gl_QUERY_BUFFER *)
  (* | GL_SHADER_STORAGE_BUFFER -> Constants.gl_SHADER_STORAGE_BUFFER *)
  | GL_TEXTURE_BUFFER -> Constants.gl_TEXTURE_BUFFER
  | GL_TRANSFORM_FEEDBACK_BUFFER -> Constants.gl_TRANSFORM_FEEDBACK_BUFFER
...
let usage_mode_to_int = function
  | GL_STREAM_DRAW -> Constants.gl_STREAM_DRAW
...
let gl_shader_source shader count str length = match length with
  | Some len -> c_gl_shader_source ~shader:shader ~count:count ~string:str ~length:len
  | None     -> c_gl_shader_source_null_length ~shader:shader ~count:count ~string:str
...
let gl_type_to_int = function
  | GL_BYTE -> Constants.gl_BYTE
  | GL_FLOAT -> Constants.gl_FLOAT
  | GL_UNSIGNED_BYTE -> Constants.gl_UNSIGNED_BYTE
  | GL_SHORT -> Constants.gl_SHORT
  | GL_UNSIGNED_SHORT -> Constants.gl_UNSIGNED_SHORT
  | GL_INT -> Constants.gl_INT
  | GL_UNSIGNED_INT -> Constants.gl_UNSIGNED_INT

external c_gl_vertex_attrib_pointer: index:int -> size:int -> t:int -> normalized:bool -> stride:int -> bool = "gl_vertex_attrib_pointer_params" "gl_vertex_attrib_pointer";;
let gl_vertex_attrib_pointer index size t normalized stride = c_gl_vertex_attrib_pointer ~index:index ~size:size ~t:(gl_type_to_int t) ~normalized:normalized ~stride:stride;;

external c_gl_compile_shader: int -> bool = "gl_compile_shader";;
let gl_compile_shader = c_gl_compile_shader;;

external c_gl_create_program: unit -> int = "gl_create_program";;
let gl_create_program = c_gl_create_program;;

external c_gl_attach_shader: int -> int -> int = "gl_attach_shader";;
let gl_attach_shader = c_gl_attach_shader

external c_gl_link_program: int -> bool = "gl_link_program";;
let gl_link_program = c_gl_link_program

external c_gl_use_program: int -> bool = "gl_use_program";;
let gl_use_program = c_gl_use_program

external c_gl_enable_vertex_attrib_array: int -> bool = "gl_enable_vertex_attrib_array";;
let gl_enable_vertex_attrib_array = c_gl_enable_vertex_attrib_array
